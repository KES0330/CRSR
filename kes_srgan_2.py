...
from tensorflow.keras.layers import Input, Dense, Reshape, Flatten, Dropout, Concatenate, AveragePooling2D
from tensorflow.keras.layers.experimental.preprocessing import Resizing
from tensorflow.keras.layers import BatchNormalization, Activation, ZeroPadding2D, Add, Lambda, Subtract
...
import sys
from kes_data_loader_2 import DataLoader
import numpy as np
...
class SRGAN():
    ...
    def build_generator(self):
        def DownScale(input):
            ...
            return tf.image.resize(input, (width, height), method=tf.image.ResizeMethod.MITCHELLCUBIC)
        def UpScale(input):
            width = (input.shape[1]*2)
            height = (input.shape[2]*2)
