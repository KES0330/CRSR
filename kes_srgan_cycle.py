...
class SRGAN():
    def __init__(self):
        ...
        self.model_name = "210205_CRSR_e3,e3,1,1"
        self.n_residual_blocks = 7
        self.Epochs = 0
        self.weights = [1e-3, 1e-3, 1, 1]
        ...
        self.combined = Model([img], [validity, validity2, CRUP_features, CRSR_features])
        self.combined.compile(loss=['binary_crossentropy', 'binary_crossentropy', 'mse', 'mse'],
    ...
    def train(self, epochs, batch_size=1, sample_interval=50):
        ...
        for epoch in range(epochs):
            ...
            if epoch == 93:
                batch_size = 1
            for batch_i, imgs in enumerate(self.data_loader.load_batch(batch_size)):
            ...
            #for batch_i in range(minibatch):
                ...
                g_loss = self.combined.train_on_batch([imgs], [valid, valid, image_features, image_features])
            ...
            new_imgs = self.data_loader.load_data(test_size)
            ...
            g_loss = self.combined.test_on_batch([new_imgs],
                                                 [valid, valid, new_image_features, new_image_features])
            ...
            test_imgs = self.data_loader.load_data(test_size, is_testing=False)
            ...
            test_g_loss = self.combined.test_on_batch([test_imgs],
                                                      [valid, valid, test_image_features, test_image_features])
    ...
    def sample_images_save(self, epoch, batch_i, batch_size=1):
        ...
        imgs = self.data_loader.load_data(batch_size=batch_size, is_testing=True)
        ...
        for idx in range(batch_size):
            ...
            img_lr = np.asarray(Image.fromarray(imgs[idx]).resize((self.lr_height, self.lr_height), self.interpolation))
            ...
            #print(np.max(fakes_cr[idx]), np.max(img_lr), np.max(imgs[idx]))
            compare1 = np.hstack((fakes_cr[idx], img_lr))
            compare1 = np.vstack((imgs[idx], compare1))

            compare2 = np.hstack((imgs[idx], fakes_crsr[idx]))
            compare2 = np.hstack((compare2, img_lrhr))

            compare3 = np.hstack((imgs[idx], fake_crup))
            compare3 = np.hstack((compare3, fakes_crsr[idx]))
            ...
            # Image.fromarray(compare3).save('images/%s/%d/%d_0compare3%d.png' % (self.model_name, epoch, batch_i, idx))
            ...
            Image.fromarray(img_lr).save('images/%s/%d/%d_6%s_lr%d.png' % (self.model_name, epoch, batch_i, self.interpolation_str, idx))
    ...
    def sample_images(self, epoch, batch_i):
        ...
        imgs = self.data_loader.load_data(batch_size=2, is_testing=True)
        fakes_cr = self.CRG.predict_on_batch(imgs)
        fakes_crsr = self.SRG.predict_on_batch(fakes_cr)
        ...
        cnt = 0
        for row in range(r):
            for col, image in enumerate([imgs, fakes_crsr, imgs_lrhr]):
    ...
    def test(self, epoch, batch_i, best_test_loss=0.):
        ...
        imgs = self.data_loader.load_select_data(dataset='./test/*')
        fakes_cr = self.CRG.predict_on_batch(imgs)
        fakes_crsr = self.SRG.predict_on_batch(fakes_cr)
        ...
        for i in range(len(imgs)):
            ...
            compare1 = np.hstack((fakes_cr[i], img_lr))
            ...
            # compare2 = np.hstack((imgs[i], fakes_crsr[i]))
            # compare2 = np.hstack((compare2, img_lrhr))
            ...
            # compare3 = np.hstack((imgs[i], fakes_crsr[i]))
            # compare3 = np.hstack((compare3, fake_crup))
            # Image.fromarray(compare3).save('images/%s/best/%d/%d_%4f/%d_%d_%dcompare3.png' % (self.model_name, epoch, batch_i, best_test_loss, epoch, batch_i, i))
...
if __name__ == '__main__':
    gan = SRGAN()
    gan.train(epochs=100, batch_size=8, sample_interval=50)
